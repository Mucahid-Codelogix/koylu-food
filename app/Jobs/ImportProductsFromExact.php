...
class ImportProductsFromExact implements ShouldQueue
{
    ...
    public function handle(ExactProductImportService $importService, ExactOnlineClient $client): void
    {
        ...
        $this->notifyUser(
            $user,
            'Artikelen geïmporteerd uit Exact',
            $result->summary(),
            false,
        );
    }
    ...
    private function notifyUser(?User $user, string $title, string $body, bool $isDanger): void
    {
        ...
        try {
            ...
            Log::warning('Exact product import notification could not be stored.', [
                'user_id' => $user->id,
                'title' => $title,
                'body' => $body,
